...
import { saveCropRecommendation } from '../services/cropService';
...
const CropDetailScreen = ({ route, navigation }: CropDetailScreenProps) => {
  ...
  return (
    <>
      ...
      {/* Save Button */}
      <View className="bg-gray-900 p-4 border-t border-gray-700">
        <TouchableOpacity 
          onPress={handleSave}
          className="bg-green-600 py-4 rounded-xl flex-row items-center justify-center"
        >
          <MaterialCommunityIcons name="bookmark-plus-outline" size={22} color="white" />
          <Text className="text-white text-lg font-bold ml-2">Save Recommendation</Text>
        </TouchableOpacity>
      </View>
    </>
  );
